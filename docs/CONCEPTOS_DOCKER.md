...
tiene 2: `./api_innovacion`, `./front_flask`.

Este es el de `api-innovacion`, sin los comentarios para verlo de un vistazo:
...
| `EXPOSE 8030` | **Documenta** en qué puerto escucha el programa | No abre nada: quien publica el puerto es el `ports:` del compose |
...
|---|---|---|
| `api-innovacion` | **Sí**, en `./api_innovacion` | Es código **suyo**: nadie más lo tiene, hay que armarlo |
...
- En la v1: `bd_v1` es un contenedor creado desde la imagen `postgres:16-alpine`,
  con el puerto interno 5432 **publicado** en el 15451 de su PC (`-p 15451:5432`).
...
flowchart LR
    ...
    subgraph PC["Su PC — Docker Desktop (el 'centro de datos')"]
        subgraph RED["red interna del compose (LAN virtual, con DNS propio)"]
            FRONTFLASK["SERVIDOR WEB (front)<br/>contenedor front-flask<br/>hostname: front-flask · escucha en 8028"]
            APIINNOVACION["SERVIDOR DE APLICACIONES<br/>contenedor api-innovacion<br/>hostname: api-innovacion · escucha en 8030"]
            POSTGRES[("SERVIDOR DE BASE DE DATOS<br/>PostgreSQL · contenedor postgres<br/>hostname: postgres · escucha en 5432")]
        end
    end
    NAV -->|"localhost:8030"| APIINNOVACION
    NAV -->|"localhost:8028"| FRONTFLASK
    APIINNOVACION -->|"postgres:5432 (DNS de Docker)"| POSTGRES
    FRONTFLASK -->|"http://api-innovacion:8030<br/>al controlador, por el NOMBRE"| APIINNOVACION
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:15451"| POSTGRES
```
...
services:                          # el mapa de TODOS los contenedores del sistema
  ...
  postgres:                        # ← este nombre es también su HOSTNAME interno
    ...
    ports:
      - "15451:5432"               # "puerto en su PC : puerto interno del contenedor"
    healthcheck:                   # cómo saber si la BD ya RESPONDE (no solo "existe")
      test: ["CMD-SHELL", "pg_isready -U paradigmas -d innovacion_local"]
      interval: 5s
      timeout: 5s
      retries: 10

  api-registros:
    build: ./api_innovacion          # esta imagen SE CONSTRUYE con el Dockerfile de esa carpeta
    volumes:
      - ./api_innovacion:/app        # el código montado: guardar un .py recarga la API sola
    ...
    ports:
      - "8030:8030"                # http://localhost:8030/docs
    environment:
      ...
      DB_POSTGRES: postgresql+asyncpg://paradigmas:paradigmas123@postgres:5432/innovacion_local
    depends_on:
      postgres:
...
1. **Dos redes de nombres**: hacia su PC, puertos publicados
   (`localhost:8030`, `localhost:15451`); entre contenedores, nombres de
...
docker network create proyecto_paradigmas_innovacion_curricular1_default

# 2. postgres
docker run -d --name paradigmas-innovacion-postgres --network proyecto_paradigmas_innovacion_curricular1_default --restart unless-stopped `
  -e "POSTGRES_USER=innovacion" `
  -e "POSTGRES_PASSWORD=Innovacion123!" `
  -e "POSTGRES_DB=innovacion_local" `
  -v pgdata:/var/lib/postgresql/data `
  -v "${PWD}/db/init.sql:/docker-entrypoint-initdb.d/init.sql:ro" `
  -p 15451:5432 postgres:16

# 3. ESPERAR a que responda de verdad… mirándolo a ojo

# 4. Construir la imagen de api-innovacion y encenderla
docker build -t api-innovacion ./api_innovacion
docker run -d --name paradigmas-innovacion-api --network proyecto_paradigmas_innovacion_curricular1_default --restart unless-stopped `
  -e "DB_POSTGRES=postgresql+asyncpg://innovacion:Innovacion123!@postgres:5432/innovacion_local" `
  -v "${PWD}/api_innovacion:/app" `
  -p 8030:8030 api-innovacion uvicorn main:app --host 0.0.0.0 --port 8030 --reload

# 5. Construir la imagen de front-flask y encenderla
docker build -t front-flask ./front_flask
docker run -d --name innovacion-front --network proyecto_paradigmas_innovacion_curricular1_default --restart unless-stopped `
  -e "URL_API_INNOVACION=http://api-innovacion:8030" `
  -v "${PWD}/front_flask:/app" `
  -p 8028:8028 front-flask
...
```yaml
URL_API: http://api-innovacion:8030
```
...
**`api-innovacion` es un nombre que solo existe dentro de la red de Docker.**
Su navegador corre en Windows, fuera de esa red, y no sabe quién es.

| Desde dónde | Qué dirección sirve | Por qué |
|---|---|---|
| **Su navegador** | `http://localhost:8030` | Está fuera de Docker. Usa el puerto **publicado** |
| **Otro contenedor** | `http://api-innovacion:8030` | Están en la misma red: se llaman por el **nombre del servicio** |
...
> **En este proyecto los dos números son iguales** (`8030:8030`), y eso
...
Si alguien cambiara esa línea por `http://localhost:8030`, el front
...
```powershell
docker network inspect proyecto_paradigmas_innovacion_curricular1_default
```
...
`docker-compose.yml` y le pega `_default`. La carpeta `proyecto_paradigmas_innovacion_curricular1` produce
la red `proyecto_paradigmas_innovacion_curricular1_default`. Tres formas de confirmarlo:
...
docker inspect innovacion-front --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}'
```
...
network proyecto_paradigmas_innovacion_curricular1_default has active endpoints (name:"innovacion-front" …)
```
...
6. En este repositorio: el `docker run` de la v1 en el
   ...
   [modelo de datos de la v1](spec_kit/versiones/v1_aliado/5_data_model.md).
