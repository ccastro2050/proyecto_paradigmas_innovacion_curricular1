...
módulo y carga las 14 filas de la tabla `aliado`. Si el motor fallara a la
...
```sql
BEGIN;
UPDATE aliado SET activo = 0 WHERE nit = …;
...
llave primaria de `aliado` impide dos fichas con el mismo `nit` —
...
aliado al tiempo. Sin aislamiento, las dos leen «está activo» y las dos
...
Un módulo académico está del lado izquierdo: un aliado no puede existir a
...
[7_quickstart.md](spec_kit/versiones/v1_aliado/7_quickstart.md), que
