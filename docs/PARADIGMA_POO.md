...
SELECT ciudad, COUNT(*) FROM aliado
...
```mermaid
flowchart TB
    subgraph PROY["El proyecto api_innovacion — multiparadigma a propósito"]
...
### 2.1 Abstracción
Quedarse con lo esencial y esconder el detalle. `IRepositorioAliado` es una
abstracción: define QUÉ se puede hacer con aliados (obtener, crear,
actualizar, eliminar) sin decir CÓMO ni DÓNDE se guardan.

### 2.2 Encapsulamiento
Cada objeto guarda su estado y expone solo operaciones. En la v1,
`RepositorioAliadoPostgreSQL` encapsula el engine de conexión y el SQL: nadie
...
es **composición sobre herencia** — y este proyecto la sigue: `ServicioAliado`
...
`IRepositorioAliado` puede ocupar el lugar de otra — el PostgreSQL real, el
...
classDiagram
    ...
    class ServicioAliado {
        -repositorio: IRepositorioAliado
        +reglas de negocio (límite mayor que 0, ...)
    }
    RepositorioAliadoPostgreSQL ..|> IRepositorioAliado : POLIMORFISMO
    RepositorioFalsoEnMemoria ..|> IRepositorioAliado : POLIMORFISMO
    ServicioAliado o-- IRepositorioAliado : COMPOSICIÓN (recibe, no hereda)
    note for IRepositorioAliado "ABSTRACCIÓN: declara QUÉ — ni una línea de CÓMO"
    note for RepositorioAliadoPostgreSQL "ENCAPSULAMIENTO: la conexión y el SQL no salen de aquí"
```
...
Cuando `ServicioAliado` manda el mensaje `crear(datos)`, NO sabe (ni le
...
modelo Pydantic: `class Aliado(BaseModel)` — Aliado ES-UN BaseModel y
...
classDiagram
    direction LR
    class ServicioMal["ServicioAliado ❌"]
    class ServicioBien["ServicioAliado ✅"]
    ServicioMal --|> RepositorioAliadoPostgreSQL : hereda del CONCRETO — quedó casado con PostgreSQL
    ServicioBien o-- IRepositorioAliado : compone la ABSTRACCIÓN — cualquier motor entra
```
...
sequenceDiagram
    ...
    participant S as ServicioAliado
    participant R as IRepositorioAliado (Protocol)
    participant BD as PostgreSQL
    Cli->>C: POST /api/aliado (JSON)
...
Aliado(codigo="PR009", nombre="Webcam", stock=5, valorunitario=120000)  # ✔ objeto válido
Aliado(codigo="PR009", nombre="Webcam", stock=-5, valorunitario=1)      # ✖ ValidationError
```

Mírelo con lentes de paradigma — Pydantic es dos paradigmas cooperando:

- Es **P.O.O.**: `Aliado` es una clase; cada petición válida se convierte en
...
1. **Frontera de entrada:** FastAPI recibe el body JSON, intenta construir el
   ...
   a la BD.
2. **Un modelo por semántica HTTP:** `Aliado` (POST: todo obligatorio),
   `AliadoReemplazo` (PUT: reemplazo completo) y `AliadoActualizar`
...
1. **El dominio se modela solo:** aliado, registro, cliente… son objetos
...
```
Aliado (Pydantic)          ← objeto de DATOS con validación (abstracción del dominio)
ServicioAliado             ← objeto de NEGOCIO; compone un IRepositorioAliado
IRepositorioAliado         ← contrato (Protocol): abstracción pura
RepositorioAliadoPostgreSQL ← implementación concreta (encapsula SQL y conexión)
RepositorioFalsoEnMemoria    ← otra implementación (¡polimorfismo!) para probar sin BD
```

El mismo `ServicioAliado` funciona con ambos repositorios sin cambiar una
línea — eso es el paradigma haciendo su trabajo. En la v3, un tercer objeto
(`RepositorioAliadoMysqlMariaDB`) entrará por la misma puerta.
...
8. En este repositorio: las interfaces y capas de la
   [v1](spec_kit/versiones/v1_aliado/3_plan.md).
