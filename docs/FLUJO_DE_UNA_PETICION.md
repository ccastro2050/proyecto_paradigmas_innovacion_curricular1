...
```
GET /api/aliado HTTP/1.1          ← un GET: verbo + ruta, sin body
```

```
POST /api/aliado HTTP/1.1         ← un POST: verbo + ruta...
...
```python
@router.get("/aliado")            # ← ESTO es "el GET de la colección"
async def listar_aliados(limite: int = 1000):
    ...

@router.post("/aliado")           # ← y ESTO es "el POST"
async def crear_aliado(aliado: Aliado):
    ...
```

El decorador registra la pareja **(verbo, ruta) → función** en una tabla de
rutas. Cuando uvicorn recibe `POST /api/aliado`, FastAPI busca esa pareja
en la tabla y llama `crear_aliado(...)`. Su función es "el POST"
...
> ruta == "/api/aliado": ...`. FastAPI la esconde — el decorador es
...
|---|---|---|
| Las 6 rutas de aliado | `controllers/aliado_controller.py` | `router = APIRouter(prefix="/api", tags=["Aliado"])` + un decorador por endpoint |
...
|---|---|---|
| **Parámetro de ruta** | Aparece en la ruta entre llaves Y como argumento | `@router.get("/aliado/{codigo}")` + `def obtener_aliado(codigo: str)` |
| **Query string** | Argumento con valor por defecto que NO está en la ruta | `def listar_aliados(limite: int = 1000)` captura `?limite=3` |
| **El body JSON** | Argumento tipado con un modelo Pydantic | `def crear_aliado(aliado: Aliado)` |
...
función un objeto `Aliado` ya construido. Si el body no cumple, responde
...
`POST /api/aliado` con `{"codigo":"PR009","nombre":"Webcam","stock":10,"valorunitario":350000}`:

```
1. uvicorn           recibe el texto HTTP y se lo pasa a FastAPI
2. FastAPI           busca (POST, /api/aliado) en la tabla de rutas
3. FastAPI/Pydantic  valida el body contra el modelo Aliado
      ├─ ¿inválido?  → responde 422 con la lista de errores y AQUÍ TERMINA
      └─ ¿válido?    → llama crear_aliado(aliado)
4. aliado_controller.crear_aliado
      try: servicio.crear(aliado.model_dump()) …
5. ServicioAliado.crear      (reglas de negocio; no conoce HTTP)
6. RepositorioAliadoPostgreSQL.crear
      INSERT INTO aliado (...) VALUES (:codigo, :nombre, ...)  ← parametrizado
...
```mermaid
flowchart TD
    A["1. uvicorn recibe<br/>el texto HTTP"] --> B["2. FastAPI encuentra<br/>POST /api/aliado<br/>en la tabla de rutas"]
...
```
1. FastAPI busca (GET, /api/aliado/{codigo}) y extrae codigo="PR001"
2. aliado_controller.obtener_aliado("PR001")
3. ServicioAliado.obtener   pide al repositorio; si llega None → lanza
                              LookupError (el except la vuelve 404)
4. RepositorioAliadoPostgreSQL   SELECT ... WHERE codigo = :codigo
...
```mermaid
flowchart LR
    A["GET /api/<br/>aliado/PR001"] --> B["controlador"] --> S["servicio"] --> R["repositorio:<br/>SELECT ... WHERE<br/>codigo = :codigo"]
    R --> E{"¿hay fila?"}
    E -->|"sí"| OK["200: el aliado<br/>en JSON"]
...
Invoke-RestMethod "http://localhost:8030/api/aliado"
Invoke-RestMethod "http://localhost:8030/api/aliado/PR001"

# POST — crear
Invoke-RestMethod -Method Post -Uri "http://localhost:8030/api/aliado" -ContentType "application/json" -Body '{"codigo":"PR009","nombre":"Webcam","stock":10,"valorunitario":350000}'

# PUT con body incompleto → error 422 (PUT exige TODO)
Invoke-RestMethod -Method Put -Uri "http://localhost:8030/api/aliado/PR009" -ContentType "application/json" -Body '{"stock":25}'

# PATCH con el MISMO body → 200 (PATCH es parcial)
Invoke-RestMethod -Method Patch -Uri "http://localhost:8030/api/aliado/PR009" -ContentType "application/json" -Body '{"stock":25}'
