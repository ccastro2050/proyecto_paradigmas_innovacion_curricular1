...
```
            EL CLIENTE (navegador, Swagger, curl)
                 │
                 │  ① GET /api/aliado/PR001
                 ▼
┌─────────────────────────────────────────────────────┐
│ CAPA 1 — CONTROLLER (HTTP)                          │
│ controllers/aliado_controller.py                  │
...
└────────────────┬────────────────────────────────────┘
                 ...
                 │     — a través de la INTERFAZ IRepositorioAliado
                 ▼
┌─────────────────────────────────────────────────────┐
│ CAPA 3 — REPOSITORIO (datos)                        │
│ repositorios/repositorio_aliado_postgresql.py     │
│ El SQL: traduce filas ↔ objetos. NO conoce HTTP.    │
│ NO decide negocio.                                  │
└────────────────┬────────────────────────────────────┘
                 │  ④ SELECT … FROM aliado WHERE codigo = 'PR001'
                 ▼
          ┌───────────────┐
          │ BASE DE DATOS │  PostgreSQL — innovacion_local
          └───────┬───────┘
                  │
...
|---|---|---|---|
| **Controller** | HTTP: rutas, códigos de estado, JSON | SQL y reglas de negocio | `controllers/aliado_controller.py` |
| **Servicio** | Las reglas del negocio (¿existe? ¿se puede?) | Saber de HTTP o del motor de BD | `servicios/servicio_aliado.py` |
| **Repositorio** | El SQL y el mapeo fila ↔ objeto | Saber de HTTP o decidir negocio | `repositorios/repositorio_aliado_postgresql.py` |
...
**El mismo viaje cuando algo sale mal** — `GET /api/aliado/PR999`:

1. El **repositorio** no encuentra la fila y devuelve `None` — un HECHO,
   sin opinión.
2. El **servicio** decide qué significa ese hecho: "ese aliado no
...
Pregunta legítima: la carpeta de modelos (`models/aliado.py (Pydantic)`) existe en el
...
- El **modelo es LO QUE VIAJA entre estaciones**: el repositorio arma un
  `Aliado` desde la fila, el servicio lo razona, el controller lo
...
flowchart LR
    ...
    M["MODELO Aliado<br/>el paquete que viaja"]
    C -.->|"lo conoce"| M
    S -.->|"lo conoce"| M
    R -.->|"lo conoce"| M
...
(`RepositorioAliadoMysqlMariaDB`) y una línea en la fábrica — controllers y
...
class RepositorioAliadoMariaDB:
    """Los mismos 5 métodos que promete IRepositorioAliado."""

# ...y el ensamblador (ÚNICO archivo tocado) elegirá el motor:
repositorio = (RepositorioAliadoMariaDB(cadena)
               if motor == "mariadb"
               else RepositorioAliadoPostgreSQL(cadena))
```

### L — Sustitución de Liskov (*Liskov Substitution*)
> Donde sirve el tipo base, debe servir CUALQUIER implementación, sin sorpresas.

**Su momento son la v3 y la v4:** los tres repositorios de aliado
...
servicio = ServicioAliado(RepositorioFalso())   # ← el servicio NI SE ENTERA
```
...
**En la v1:** `IRepositorioAliado` tiene exactamente los 5 métodos del CRUD
de aliado — no un `IRepositorioUniversal` con 40 métodos. Cuando la v2
agregue persona, tendrá SU interfaz.

```python
# ✅ La interfaz de la v1: SOLO los 5 métodos del CRUD de aliado
...
**En la v1:** `ServicioAliado` recibe **la interfaz** por constructor; solo
...
class ServicioAliado:
    def __init__(self, repositorio: IRepositorioAliado):   # ← interfaz, no clase
        self._repositorio = repositorio

# El ÚNICO lugar que conoce la clase concreta (v3 lo convertirá en fábrica):
def crear_servicio_aliado() -> IServicioAliado:
    repositorio = RepositorioAliadoPostgreSQL(os.environ["DB_POSTGRES"])
    return ServicioAliado(repositorio)
```
...
| **Inyección de dependencias** | creacional (IoC) | los constructores + el ensamblador (`crear_servicio_aliado()`) |
...
# ServicioAliado lo usa SIN saber si detrás hay PostgreSQL o un
...
```python
class ServicioAliado:
    def __init__(self, repositorio: IRepositorioAliado):  # ← llega armado
        self._repositorio = repositorio

# El ÚNICO lugar que conoce la clase concreta es el ensamblador:
def crear_servicio_aliado() -> IServicioAliado:
    return ServicioAliado(RepositorioAliadoPostgreSQL(cadena))
```

### DTO por verbo — el body aterriza en un modelo que solo valida forma

```python
class Aliado(BaseModel):                       # POST: todo obligatorio
    codigo: str = Field(min_length=1, max_length=20)
    stock: int = Field(ge=0)
    # Un body inválido muere en 422 ANTES de tocar servicio o BD.

class AliadoActualizar(BaseModel):             # PATCH: todo opcional
    ...
```
...
def crear_repositorio_aliado(motor: str) -> IRepositorioAliado:
    if motor == "mariadb":
        return RepositorioAliadoMariaDB(cadena_mariadb)
    return RepositorioAliadoPostgreSQL(cadena_postgres)
...
(`RepositorioAliadoPostgreSQL`, el `RepositorioFalso` de la prueba — y
...
7. En este repositorio: el [plan de la v1](spec_kit/versiones/v1_aliado/3_plan.md)
